...
All animations are realized in the file *Animations.js* that contains all the functions needed to produce the desired effect, by combining tweens of different parts of the model that is passed as input. A useful tool that allowed to quickly add a very basic user interface to interact with the 3D scene is *lil-gui*, a drop-in replacement for *dat.gui*.
